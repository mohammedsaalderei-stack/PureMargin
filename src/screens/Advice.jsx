...
function Confidence({ level }) {
  ...
  const { t } = useLang();
  const map = {
    high: { bg: C.irisWash, fg: C.irisDeep },
    medium: { bg: C.lilacWash, fg: C.iris },
    low: { bg: "rgba(196,66,110,.12)", fg: C.rose },
  };
...
function Block({ icon: Icon, title, tone, children }) {
  const C = useC();
  const tones = {
    impact: { bg: C.irisWash, fg: C.irisDeep },
    watch: { bg: C.lilacWash, fg: C.iris },
    caution: { bg: "rgba(196,66,110,.1)", fg: C.rose },
    plain: { bg: C.bone, fg: C.slate },
  };
  const s = tones[tone] || tones.plain;
  return (
    <div className="rounded-lg p-4" style={{ background: s.bg }}>
    ...
    >
      <div className="p-5 md:p-6 pb-0">
        ...
        <p className="text-[11px] leading-relaxed mb-4" style={{ color: C.slate }}>
          {fill(t.advice.basisNote, item.basis)}
          {item.confidence === "low" ? t.advice.basisStale : ""}
        </p>
      ...
      <div className="px-5 md:px-6 space-y-3">
        <Block icon={TrendingUp} title={t.advice.impact} tone="impact">
          {copy.impact}
        </Block>
      </div>
      ...
      <div className="px-5 md:px-6 py-5">
        <div className="text-xs font-semibold mb-3" style={{ color: C.slate }}>
          {t.advice.figuresTitle}
        </div>
        <div className="grid grid-cols-3 gap-3" style={{ borderBottom: `1px solid ${C.hairline}`, paddingBottom: 16 }}>
          {item.figures.map((f) => (
            <div key={f.key}>
              <div className="text-[11px] mb-1" style={{ color: C.slate }}>
                {t.watch[FIGURE_LABEL[f.key]] || t.advice[f.key] || f.key}
              </div>
              <div className="display font-extrabold text-base leading-none">
                {MONEY_KEYS.has(f.key) ? (
                  ...
                  <span dir="ltr">
                    {typeof f.value === "number" ? f.value.toLocaleString("en-AE") : f.value}
                    {PERCENT_KEYS.has(f.key) ? "%" : ""}
                  </span>
                )}
              </div>
            </div>
        ...
        <div className="text-[11px] mt-3" style={{ color: C.slate }} dir="ltr">
          {t.advice.period}: {item.period.from} — {item.period.to}
        </div>
      </div>
      ...
      <div className="px-5 md:px-6 space-y-3 pb-5">
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          ...
          <Block icon={Timer} title={t.advice.measure} tone="plain">
            {fill(t.advice.measureIn, { days: item.measureInDays })}
          </Block>
        </div>
      ...
      <div className="px-5 md:px-6 pb-5 mt-auto">
        <button
          onClick={() => onAsk(`${title} — ${t.advice.ask}`)}
    ...
    <div className="h-full overflow-y-auto">
      <div className="max-w-6xl mx-auto px-4 md:px-8 py-5 md:py-8">
        <div className="text-center mb-6">
          ...
          <h2 className="display text-2xl md:text-3xl font-extrabold">{t.advice.title}</h2>
          ...
          <p className="text-[11px] mt-3" style={{ color: C.slate }}>
            {t.advice.lastRun}:{" "}
            <span dir="ltr">{data.updatedAt ? new Date(data.updatedAt).toLocaleString(localeFor(lang)) : "—"}</span>
          </p>
        </div>

        <div className="grid gap-4 lg:gap-5 lg:grid-cols-2">
          {items.map((item, i) => (
            <AdviceCard key={item.id} item={item} index={i} shown={i < shown} onAsk={onAsk} />
          ))}
        </div>
      </div>
    </div>
