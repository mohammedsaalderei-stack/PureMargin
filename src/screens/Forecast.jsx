...
export default function Forecast({ data }) {
  ...
  const scenarios = [
    ...
    { key: "optimistic", label: t.forecast.good, color: C.lilac, note: t.forecast.goodNote },
  ];
  ...
  return (
    <div className="h-full overflow-y-auto">
      <div className="max-w-6xl mx-auto px-4 md:px-8 py-5 md:py-8 space-y-4 md:space-y-5">
        <div>
          <h2 className="display text-2xl md:text-3xl font-extrabold">{t.forecast.title}</h2>
          ...
          {scenarios.map((s) => {
            ...
            return (
              <button
                ...
                className="text-start rounded-xl p-4 md:p-5 transition"
                style={{
                  background: on ? C.surface : "transparent",
                  border: `1.5px solid ${on ? s.color : C.hairline}`,
                }}
              >
                <div className="text-xs font-semibold mb-1.5" style={{ color: s.color }}>{s.label}</div>
                <div className="display figure text-xl md:text-2xl font-extrabold leading-none">
                  <Money value={forecast[s.key]} />
                </div>
          ...
          <div className="chart" style={{ height: 240 }}>
            <ResponsiveContainer>
              <AreaChart data={forecast.series} margin={{ top: 8, right: 8, left: -18, bottom: 0 }}>
                ...
                <XAxis
                  dataKey="day" tick={{ fontSize: 11, fill: C.slate }}
        ...
        <div className="panel p-5 md:p-6" style={{ background: C.irisWash }}>
