    ...
    <section id={id} ref={ref} className="px-6 md:px-10 lg:px-16 py-16 md:py-24">
      <div className="max-w-6xl mx-auto">
        <div className={shown ? "rise" : "opacity-0"}>
          ...
          <h2 className="display text-3xl md:text-4xl lg:text-5xl font-extrabold max-w-3xl leading-[1.15]">
            {title}
          </h2>
          {lead && (
...
export default function Landing({ onSignIn, onRegister, onPricing }) {
  ...
  return (
    <div className="min-h-full substrate">
      ...
      >
        ...
        {menu && (
          <div className="md:hidden px-6 pb-4 flex flex-col gap-3 text-sm font-medium">
            ...
            <button
              onClick={onSignIn}
              className="mt-1 px-4 py-2.5 rounded-lg font-semibold"
              style={{ background: C.iris, color: C.onPrimary }}
            >
              {t.common.signIn}
            </button>
      ...
      {/* Hero */}
      <section id="story" className="px-6 md:px-10 lg:px-16 pt-14 pb-16 md:pt-20 md:pb-24">
        <div className="max-w-6xl mx-auto grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          <div className="rise">
            <div className="data text-xs uppercase tracking-[0.18em] mb-5" style={{ color: C.iris }}>
              {t.hero.eyebrow}
            </div>
            <h1 className="display text-4xl md:text-5xl lg:text-6xl font-extrabold leading-[1.1]">
              {t.hero.title}
            </h1>
      ...
      {/* Data journey */}
      <div style={{ background: C.surface, borderTop: `1px solid ${C.hairline}`, borderBottom: `1px solid ${C.hairline}` }}>
        ...
        <div className="space-y-5">
          {t.capabilities.items.map((cap, i) => {
            ...
            return (
              <div
                key={cap.name}
                className="rounded-xl p-6 md:p-8 grid md:grid-cols-[1fr_1.4fr] gap-6 md:gap-10"
                style={{ background: C.surface, border: `1px solid ${C.hairline}` }}
              >
                <div>
                  <div className="flex items-center gap-2.5 mb-3">
                    <div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ background: C.iris }}>
                      <Icon size={17} color={C.onPrimary} />
                    </div>
                    <span className="display font-extrabold text-2xl">{cap.name}</span>
                    <span className="data text-xs ms-auto" style={{ color: C.slate }} dir="ltr">
      ...
      <div id="decision" style={{ background: C.surface, borderTop: `1px solid ${C.hairline}` }}>
              ...
              return (
                <div key={item.t} className="panel p-6">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ background: C.irisWash }}>
                      <Icon size={17} style={{ color: C.iris }} />
                    </div>
      ...
      <div style={{ background: C.panel }}>
        <section id="steps" className="px-6 md:px-10 lg:px-16 py-16 md:py-24">
          <div className="max-w-6xl mx-auto">
            ...
            <h2
              className="display text-3xl md:text-4xl lg:text-5xl font-extrabold max-w-3xl leading-[1.15]"
            ...
            <div className="mt-12 grid md:grid-cols-3 gap-8 md:gap-6">
              {t.steps.items.map((step, i) => (
                <div key={step.t} className="md:pe-6" style={{ borderTop: "1px solid rgba(255,255,255,.15)" }}>
                  <div className="data text-sm pt-5 mb-3" style={{ color: C.lilac }} dir="ltr">
                    {String(i + 1).padStart(2, "0")}
                  </div>
      ...
      <section className="px-6 md:px-10 lg:px-16 py-20 md:py-28">
        <div className="max-w-2xl mx-auto text-center">
          <h2 className="display text-3xl md:text-4xl font-extrabold leading-[1.2]">{t.close.title}</h2>
          <p className="mt-4 text-base" style={{ color: C.slate }}>{t.close.lead}</p>
          <div className="mt-8 flex flex-wrap gap-3 justify-center">
            <button
              onClick={onSignIn}
              className="px-7 py-3.5 rounded-lg font-semibold inline-flex items-center gap-2"
              style={{ background: C.iris, color: C.onPrimary }}
            >
